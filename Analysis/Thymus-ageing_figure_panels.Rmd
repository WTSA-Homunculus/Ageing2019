...
thymic.cell.counts <- do.call(rbind.data.frame,
                              list("Total"=thymus.total.cellularity, "Tcell"=tcell.total.cellularity, 
                                   "cTEC.Total"=ctec.total.cellularity))
# remove empty rows 
...
mean.thymo.count <- apply(thymus.total.cellularity[, c(2:5)], 1, mean)
sd.thymo.count <- apply(thymus.total.cellularity[, c(2:5)], 1, sd)
...
ensembl <- useMart("ensembl", "mmusculus_gene_ensembl", host="grch37.ensembl.org", path="/biomart/martservice")
...
thymus.meta$Interpretation[thymus.meta$TFIDF.Cluster == "8"] <- "eTEC1"
thymus.meta$Interpretation[thymus.meta$TFIDF.Cluster == "10"] <- "eTEC2"
...
names(inter.cols) <- c("Post-Aire mTEC", 'Intertypical TEC',' Mature cTEC', 'Tuft-like mTEC', 
                       'Proliferating TEC', 'Mature mTEC', 'eTEC1', 'Perinatal cTEC', 'eTEC2')
```
...
```{r}
thymus.meta$AgeInt <- as.numeric(as.character(thymus.meta$AgeInt))
...
markers.melt.merge$variable <- factor(markers.melt.merge$variable,
                                      levels=c("Krt80", "Spink5", "Ccl21a", "Krt5", "Prss16", "Cxcl12", "Avil", "Trpm5", "Ccna2", "Pbk",
                                               "Aire", "Cd52", "Cd177", "Car8", "Gper1", "Syngr1", "Dpt", "Sod3"))
...
marker.annot$Cluster[marker.annot$TFIDF.Cluster == "8"] <- "eTEC1"
marker.annot$Cluster[marker.annot$TFIDF.Cluster == "10"] <- "eTEC2"

names(inter.cols) <- c("Post-Aire mTEC", 'Intertypical TEC','Mature cTEC', 'Tuft-like mTEC', 
                       'Proliferating TEC', 'Mature mTEC', 'eTEC1', 'Perinatal cTEC', 'eTEC2')

marker.annot$Cluster <- factor(marker.annot$Cluster,
                                 levels=c("eTEC2", "eTEC1", "Perinatal cTEC",
...
alltec.res$Interpretation[alltec.res$Cluster == "Cluster8"] <- "eTEC1"
alltec.res$Interpretation[alltec.res$Cluster == "Cluster10"] <- "eTEC2"
...
quad.alltec.res$Interpretation[quad.alltec.res$Cluster == "Cluster8"] <- "eTEC1"
quad.alltec.res$Interpretation[quad.alltec.res$Cluster == "Cluster10"] <- "eTEC2"
...
all.scaled.sum$Interpretation[all.scaled.sum$Cluster == "Cluster8"] <- "eTEC1"
all.scaled.sum$Interpretation[all.scaled.sum$Cluster == "Cluster10"] <- "eTEC2"
...
alltec.freq$Interpretation[alltec.freq$Cluster == "Cluster8"] <- "eTEC1"
alltec.freq$Interpretation[alltec.freq$Cluster == "Cluster10"] <- "eTEC2"
```
...
tra.down.merge <- merge(tra.down.df, thymus.meta[, c("Sample", "PC1", "PC2", "Dim1", "Dim2", "SumFactor", "AgeInt", 
...
# filter low abundance clusters?
maturemTEChi.meta$AgeInt <- maturemTEChi.meta$AgeInt
...
maturemTEChi.volcano <- ggplot(maturemTEChi.res, aes(x=logFC, y=-log10(FDR), fill=Sig)) +
  ...
  geom_point(shape=21, size=3) +
  scale_x_continuous(limits=c(-0.02, 0.02), oob=squish) +
...
ggsave(maturemTEChi.volcano,
       filename="~/Dropbox/AgeingExperiment/Paper/Figures/Figure4/Mature_mTEC-GeneAbundance-volcano.png",
