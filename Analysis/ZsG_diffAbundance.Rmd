...
zsg.meta$Rep <- gsub(zsg.meta$CellOrigin, pattern="Run[1-2]", replacement="")
```
...
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster4")] <- "iTEC.1"
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster7")] <- "iTEC.2"
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster10")] <- "iTEC.3"
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster11")] <- "iTEC.4"
...
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster23")] <- "eTEC1"
...
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster8")] <- "New.TEC.1"
thymus.res$TECtype[thymus.res$Cluster %in% c("Cluster21")] <- "New.TEC.2"
...
clust.prop.df$Age <- factor(clust.prop.df$Age,
                            levels=c("Wk1", "Wk4", "Wk16"),
                            labels=c("Wk1", "Wk4", "Wk16"))
clust.prop.df$Rep <- unlist(lapply(strsplit(clust.prop.df$Sample, fixed=TRUE, split="_"), FUN=function(X) paste0(X[3])))
...
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster4")] <- "iTEC.1"
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster7")] <- "iTEC.2"
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster10")] <- "iTEC.3"
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster11")] <- "iTEC.4"
...
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster23")] <- "eTEC1"
...
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster8")] <- "New.TEC.1"
clust.prop.df$TECtype[clust.prop.df$ZsG.Cluster %in% c("Cluster21")] <- "New.TEC.2"
...
```{r}
prop.1st <- table(zsg.meta$SortType[grepl(zsg.meta$Rep, pattern="1st")])[1]/table(zsg.meta$SortType[grepl(zsg.meta$Rep, pattern="1st")])[2]
...
ggsave(weighted.fdr.p,
       filename="~/Dropbox/AgeingExperiment/ZsG_10X_Exp/ClusterAbundance_Downsample_ZsG_interaction-jitter.pdf",
