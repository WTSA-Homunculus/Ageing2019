...
for(i in seq_along(1:length(cell_barcodes))){
  ...
  sname <- unlist(lapply(strsplit(cell_barcodes[[i]][1], fixed=TRUE, split="_"), FUN=function(C) paste(C[1:4], collapse="_")))
...
writeMM(intest_counts,
        file=paste0("/nfs/research1/marioni/mdmorgan/Thymus_ZsG/", args[1], "_counts-HTO.tsv"))
write.table(intest_ids,
            file=paste0("/nfs/research1/marioni/mdmorgan/Thymus_ZsG/", args[1], "_counts_colnames-HTO.tsv"),
            sep="\t", row.names=FALSE, quote=FALSE)
write.table(rownames(intest_counts),
            file=paste0("/nfs/research1/marioni/mdmorgan/Thymus_ZsG/", args[1], "_counts_rownames-HTO.tsv"),
            sep="\t", row.names=FALSE, quote=FALSE)
