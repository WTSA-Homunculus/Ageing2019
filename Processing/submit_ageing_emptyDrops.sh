...
for fle in `ls $SAMPDIR | grep "^Ageing*" | grep "HTO" | grep -v "tsv"`;
do
    echo $fle
    LOGERROR=$LOGDIR/$fle.err
    LOGOUT=$LOGDIR/$fle.out

    JOB="bsub -R "rusage[mem=32000]" -M 64000 -T 15000 -q research-rh74 -J $fle -e $LOGERROR -o $LOGOUT R CMD $SRCDIR/ZsG_emptyDrops_HTO.R $fle"
    echo $JOB
    eval $JOB
done
